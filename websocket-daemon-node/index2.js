    ...
    switch (attack.skill) {
      case 'earth': {
        ...
        for (const enemy of enemies) {
          ...
          console.log(`    ${enemyName}: HP ${oldHp} → ${newHp} (damage: ${actualDamage})`);
      ...
      case 'fire': {
        ...
        console.log(`     Defender: ${defenderName}, current HP=${defender.hp}, shield=${defender.shield}, defence=${defenderStats.defence}`);
        ...
        let actualDamage = fireDamage;
        let currentShield = defender.shield || 0;
        console.log(`     Initial shield: ${currentShield}`);
    ...
    for (const battleUser of updatedUsers) {
      if (battleUser.user_id) {
        ...
        if (userRows[0]) {
          ...
          await connection.execute(
            "UPDATE user SET exp = ?, level = ?, up = ?, points = ?, kr = ?, in_battle = NULL, battle_id = NULL WHERE id = ?",
            [newExp, newLevel, newUp, newPoints, newKr, battleUser.user_id]
          );
          
          const levelUpText = (newLevel > currentLevel || newUp > currentUp) ? ` Повышение до ${newLevel}.${newUp} UP!` : '';
          console.log(`  ✨ User ${battleUser.user_id}: +${expGain} exp, +${pointsGained} points, +${krGained} kr, level ${currentLevel}.${currentUp} → ${newLevel}.${newUp} (total damage: ${totalDamage})`);
